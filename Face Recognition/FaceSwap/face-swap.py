...
textureCoords = utils.getFaceTextureCoords(
    textureImg, mean3DShape, blendshapes, idxs2D, idxs3D, detector, predictor)
renderer = FaceRendering.FaceRenderer(
    cameraImg, textureImg, textureCoords, mesh)

while True:
    cameraImg = cap.read()[1]
    shapes2D = utils.getFaceKeypoints(
        ...
        for shape2D in shapes2D:
            ...
            if drawOverlay:
                ...
                drawProjectedShape(cameraImg, [
                                   mean3DShape, blendshapes], projectionModel, mesh, modelParams, lockedTranslation)
    ...
    if key == ord('r'):
        if writer is None:
            ...
            writer = cv2.VideoWriter("../out.avi", cv2.VideoWriter_fourcc(
                'X', 'V', 'I', 'D'), 25, (cameraImg.shape[1], cameraImg.shape[0]))
