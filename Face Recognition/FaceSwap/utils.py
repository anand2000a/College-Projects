...
def getFaceKeypoints(img, detector, predictor, maxImgSizeForDetection=640):
    ...
    if max(img.shape) > maxImgSizeForDetection:
        ...
        scaledImg = cv2.resize(
            img, (int(img.shape[1] * imgScale), int(img.shape[0] * imgScale)))
    ...
    shapes2D = []
    for det in dets:
        faceRectangle = rectangle(int(det.left() / imgScale), int(
            det.top() / imgScale), int(det.right() / imgScale), int(det.bottom() / imgScale))
...
def getFaceTextureCoords(img, mean3DShape, blendshapes, idxs2D, idxs3D, detector, predictor):
    ...
    modelParams = projectionModel.getInitialParameters(
        mean3DShape[:, idxs3D], keypoints[:, idxs2D])
    ...
    textureCoords = projectionModel.fun(
